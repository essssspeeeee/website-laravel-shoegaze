    ...
    <div class="min-h-screen bg-slate-50 py-10">
                                ...
                                @endif
                                        ...
                                        <div class="mt-5 rounded-[24px] border-2 border-green-200 bg-green-50 p-4">
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    @php
                                                        $proofRaw = trim($order->proof_image ?? '');
                                                        $proofImageUrl = null;

                                                        if ($proofRaw !== '') {
                                                            $proofRaw = ltrim($proofRaw, '/');
                                                            if (str_starts_with($proofRaw, 'storage/')) {
                                                                $proofRaw = substr($proofRaw, strlen('storage/'));
                                                            }

                                                            $candidates = [
                                                                $proofRaw,
                                                                'storage/' . $proofRaw,
                                                                'proof_payments/' . basename($proofRaw),
                                                                'payments/' . basename($proofRaw),
                                                            ];

                                                            foreach (array_unique($candidates) as $candidate) {
                                                                if (\Illuminate\Support\Facades\Storage::disk('public')->exists($candidate)) {
                                                                    $proofImageUrl = asset('storage/' . $candidate);
                                                                    break;
                                                                }

                                                                if (file_exists(public_path($candidate))) {
                                                                    $proofImageUrl = asset($candidate);
                                                                    break;
                                                                }
                                                    ...
                                                    @endphp
                                                    @if($proofImageUrl)
                                                        <img src="{{ $proofImageUrl }}" alt="Bukti Pembayaran" class="mt-3 w-full max-w-xs rounded-lg object-cover border border-green-200" />
                                                    @else
                                                        <div class="mt-3 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                                                            Gambar bukti tidak ditemukan.
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    @else
                                        <form action="{{ route('orders.upload', $order->id) }}" method="POST" enctype="multipart/form-data" class="mt-5" id="proof-upload-form">
                                            @csrf
                                            <div class="space-y-4">
                                                <div class="rounded-[24px] border-2 border-dashed border-slate-300 bg-slate-50 p-6">
                                                    <label for="file-input" class="flex flex-col items-center justify-center gap-4 text-center cursor-pointer">
                                                        ...
                                                        <div>
                                                            <p class="text-base font-semibold text-slate-600">Klik untuk memilih file bukti pembayaran</p>
                                                            <p class="text-xs text-slate-500">Gambar JPEG, PNG, GIF (Maks. 2MB)</p>
                                                        </div>
                                                        <span class="inline-flex items-center justify-center rounded-full bg-red-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-red-700 transition duration-200">
                                                            Pilih File
                                                        </span>
                                                    </label>
                                                    <input type="file" name="payment_proof" accept="image/*" required class="mt-4 w-full rounded-lg border border-slate-300 bg-white p-3 text-sm text-slate-700" id="file-input">
                                                </div>
                                                ...
                                                <button type="submit" class="w-full inline-flex items-center justify-center rounded-full bg-red-600 px-6 py-3 text-sm font-semibold text-white hover:bg-red-700 transition duration-200">
        ...
        <!-- Modal QRIS -->
        <div id="qris-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div class="bg-white rounded-[32px] shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95">
                ...
                <div class="p-6 sm:p-8">
                    <div class="bg-slate-50 rounded-[24px] p-6 flex items-center justify-center mb-4">
                        <img src="{{ asset('img/payments/qris.png') }}" alt="Kode QRIS" class="w-full max-w-xs rounded-lg object-contain" />
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('[data-modal-toggle]').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (modal) {
                modal.classList.remove('hidden'); // 🔥 pakai ini (lebih aman)
            }
        });
    });
